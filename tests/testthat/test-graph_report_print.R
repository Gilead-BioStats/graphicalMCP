test_that("printing Bonferroni/Simes closure test", {
  ...
  expect_snapshot(test_graph(par_gate, rep(.01, 4), test_types = "s"))
...
test_that("printing parametric closure test", {
  ...
  expect_snapshot(
    test_graph(par_gate, rep(.01, 4), test_types = "p", corr = diag(4))
  )
...
test_that("printing Bonferroni sequential results", {
  expect_snapshot(bonferroni_sequential(simple_successive_1(), rep(.01, 4)))
})
