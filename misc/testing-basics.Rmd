...
graph_test_closure(par_gate,
  ...
  groups = list(1, 2, 3:4),
  test_types = c("b", "b", "p"),
  corr = list(NA, NA, rbind(c(1, .5), c(.5, 1)))
)
```
...
graph_test_closure(
  ...
  p = pvals,
  alpha = .05,
  corr = rep(list(matrix(1, 1, 1)), 4), # Correlation matrix doesn't matter when each group is size 1
  groups = list(1, 2, 3, 4),
  test_types = rep("p", 4)
)
```
...
graph_test_closure(
  ...
  p = pvals,
  alpha = .05,
  corr = list(NA, rbind(c(1, .5), c(.5, 1))),
  groups = list(1:2, 3:4),
  test_types = c("s", "p")
)
```
