...
simfunc <- function(nSim, a1, a2, g1, g2, rh, t1, t2, t3, t4, Gr) {
  al <- c(a1, a2, 0, 0)
  G <- rbind(c(0, g1, 1 - g1, 0),
             c(g2, 0, 0, 1 - g2),
             c(0, 1, 0, 0),
             c(1, 0, 0, 0))
  corMat <- rbind(c(1, 0.5, rh, rh / 2),
                  c(0.5, 1, rh / 2, rh),
                  c(rh, rh / 2, 1, 0.5),
                  c(rh / 2, rh, 0.5, 1))
  mean <- c(t1, t2, t3, t4)
...
atlst1 <- as.numeric(lapply(outList, function(x)
  x$PowAtlst1))
locpow <-
  do.call("rbind", lapply(outList, function(x)
    x$LocalPower))
...
for (i in 1:14) {
  ...
  out_list[[i]] <- sim_func(
    ...
    # Base edges
    gamma_props = gamma_props[[i]],
    # Gamma edge locations
    gamma = gammas[i, ],
    # Gamma edge values
    rho = rhos[[i]],
    # Covariance base value
    theta = thetas[i, ]
  )
}
...
## summarize data as in Stat Med paper Table I
atlst1 <-
  as.numeric(lapply(out_list, function(x)
    x$power$power_at_least_1))
locpow <-
  do.call("rbind", lapply(out_list, function(x)
    x$power$power_local))
