...
motivating_example <- function(hyp_4, gamma, names = hyp_names) {
  eps <- .0001
  
  hypotheses <- c(hyp_4, 1 - hyp_4, 0, 0, 0, 0)
...
test_graph_closure(g_complex, <calculated_p_values>, alpha = .025)
```
...
Using testing methods other than Bonferroni can add power to reject more hypotheses as a result of these other methods being less conservative in controlling the Type-I error rate, but it comes with a speed cost. This is barely noticeable for a single test, but it does slow things down for power calculations. [link to speed vignette, if we keep it around] Currently the Bonferroni, parametric, and Simes classes of tests have been implemented.
...
power_scenario <- function(hyp_4, gamma, s_corr, marginal_power) {
    calculate_power(
      graph = motivating_example(hyp_4, gamma),
      alpha = .025,
      test_groups = list(1:2, c(3, 5), c(4, 6)),
      test_types = c("p", "s", "s"),
      test_corr = t_corr,
...
```{r test-bonferroni}
test_graph_shortcut(example_.5_.5, p_mean_diff_from_placebo, alpha = .025, verbose = TRUE, critical = TRUE)
```
...
```{r pres-examples-1}
shortcut_example <- test_graph_shortcut(
  example_.5_.5,
  c(.0123, .0132, .026, .007, .012, .012),
  alpha = .025,
  verbose = TRUE,
  critical = TRUE
)
...
```{r pres-examples-2}
closure_example <- test_graph_closure(
  example_.5_.5,
  c(.0129, .0132, .026, .007, .012, .012),
  alpha = .025,
  groups = list(1:2, c(3, 5), c(4, 6)),
...
print(closure_example, precision = 4)
```
...
marginal_power <- c(2.7, 2.8, 1.9, 2.1, 1.7, 2.5)

power_example <- calculate_power(
  example_.5_.5,
  alpha = .025,
  test_groups = list(1:2, c(3, 5), c(4, 6)),
  test_types = c("parametric", "simes", "simes"),
...
print(power_example, precision = 4)
```
...
if (file.exists(here("./vignettes/data/power_timing.rds"))) {
  ...
  power_timing <- bench::mark(
    `gMCP shortcut` = calcPower(graph = as_gmcp_graph(example_.5_.5),
                                alpha = .025,
                                mean = marginal_power, corr.sim = s_corr,
                                n.sim = 2^13),
    `graphicalMCP shortcut` = calculate_power(example_.5_.5,
                                              sim_n = 2^13,
                                              marginal_power = marginal_power,
                                              sim_corr = s_corr),
    `graphicalMCP closure` = calculate_power(example_.5_.5,
                                              ...
                                              marginal_power = marginal_power,
                                              sim_corr = s_corr),
    `gMCP closure` = calcPower(graph = as_gmcp_graph(example_.5_.5),
                               alpha = .025,
                               mean = marginal_power, corr.sim = s_corr,
                               n.sim = 2^13,
  ...
  power_timing$median_scale <- power_timing$median * 12.2
