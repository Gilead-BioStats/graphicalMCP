...
shortcut_example <- graph_test_shortcut(
  example_.5_.5,
...
```{r pres-examples-2}
closure_example <- graph_test_closure(
  example_.5_.5,
...
power_example <- graph_calculate_power(
  example_.5_.5,
  alpha = .025,
  test_groups = list(1:2, c(3, 5), c(4, 6)),
  test_types = c("parametric", "simes", "simes"),
...
if (file.exists(here("vignettes/data/power_timing.rds"))) {
  ...
  power_timing <- bench::mark(
    ...
    `graphicalMCP shortcut` = graph_calculate_power(example_.5_.5,
      sim_n = 2^13,
      marginal_power = marginal_power,
      sim_corr = s_corr
    ),
    `graphicalMCP closure` = graph_calculate_power(example_.5_.5,
