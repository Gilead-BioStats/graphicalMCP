...
```{r study-graph}
motiv_ex_.5_.5 <- motivating_example(.5, .5)
...
```{r test-bonferroni}
test_graph_shortcut(motiv_ex_.5_.5, p_mean_diff_from_placebo, alpha = .025, verbose = TRUE, critical = TRUE)
```
...
```{r pres-examples-2}closure_example <- test_graph_closure(
  motiv_ex_.5_.5,
