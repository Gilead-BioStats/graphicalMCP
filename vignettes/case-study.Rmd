...
```{r test-bonferroni}
test_graph_shortcut(example_.5_.5, p_mean_diff_from_placebo, alpha = .025)
```
...
closure_example2 <- closure_example
closure_example2$inputs$corr <- NULL
print(closure_example2, precision = 4)
```
...
power_example <- calculate_power(
  ...
  marginal_power = marginal_power,
  sim_corr = s_corr,
  sim_success = list(`1 and 2` = \(x) x[1] & x[2]),
  verbose = TRUE
)
