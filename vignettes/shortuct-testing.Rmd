...
$H_2$ are rejected - we can specify `verbose = TRUE` and obtain it via `test_results_verbose$details$results`. # I am not sure if they can be retrieved easily
...
test_results_verbose$details$results # To be discussed
```
...
significance level. The adjusted significance levels are calculated in the same order as adjusted p-values: $H_2 \rightarrow H_1 \rightarrow H_4$,
and there are four steps of checking for rejections. First, $H_2$ is rejected at an adjusted 
...
succ <- setNames(
  succ,
  c("H1", "Expected no. of rejections", "AtLeast1", "All", "H1andH2",
    "(H1andH3)or(H2andH4)")
)
```
