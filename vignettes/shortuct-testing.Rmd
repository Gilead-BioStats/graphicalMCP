...
plot(g, layout = "grid", vertex.size = 30)
```
...
test_results_verbose <- graph_test_shortcut(g, p = p_values, alpha = 0.025,
                                            verbose = TRUE)
...
orders
graph_update(g, delete = orders[[2]])$intermediate_graphs[[3]] # Intermediate graphs following the order of H2 and H4
```
...
unpooled_variance <- prop[-1] * (1 - prop[-1]) / sample_size[-1] +  prop[1] * (1 - prop[1]) / sample_size[1]

noncentrality_parameter_primary <- -(prop[-1] - prop[1]) / sqrt(unpooled_variance)

power_marginal_primary <- pnorm(qnorm(alpha, lower.tail = FALSE), noncentrality_parameter_primary, 1, lower.tail = FALSE
)
...
set.seed(1234)
power_output <- graph_calculate_power(g,
  alpha = .025,
  sim_corr = corr,
  sim_n = 1e5,
...
set.seed(1234)
power_verbose_output <- graph_calculate_power(g,
  alpha = .025,
  sim_corr = corr,
  sim_n = 1e5,
  power_marginal = c(power_marginal_primary, power_marginal_secondary),
...
print(power_verbose_output$details)

print(power_verbose_output$details, indent = 4, precision = 6)
```
