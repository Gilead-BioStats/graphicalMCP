...
### This error is from a previous version - Install the latest version on this branch by pressing "Ctrl + Shift + B" in the RStudio project
...
plot(fallback_graph_2005, nrow = 1, asp = .1,
...
plot(fallback_graph_2008, nrow = 1, asp = .1, eps = .0001,
     edge_curves = c("pairs" = -4, "H3|H1" = -5))
p_values <- runif(m, 0, alpha)
...
plot(serial_gatekeeping_graph, nrow = 1, asp = .1, eps = .0001,
     edge_curves = c("pairs" = -4, "H3|H1" = -5))
p_values <- runif(m, 0, alpha)
...
plot(parallel_gatekeeping_graph)
p_values <- runif(m, 0, alpha)
...
plot(parallel_gatekeeping_improved_graph, eps = .0001)
p_values <- runif(m, 0, alpha)
