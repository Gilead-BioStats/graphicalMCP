...
table_benchmarks <- function(df, title, unit) {
  df_benchmarks_gw %>%
    ...
    gt() %>%
    tab_header(title) %>%
    fmt_number(pattern = paste0("{x}", unit)) %>%
    tab_spanner("Number of hypotheses", 2:ncol(df)) %>%
...
if (file.exists(here("vignettes/data/df_benchmarks_power_closure.rds"))) {
  ...
  df_benchmarks_power_closure <- do.call(rbind, list_benchmarks_power_closure)[keep_cols] %>%
    filter(!(char_expression == "gMCP closure parametric (R)" & size > 5))
...
df_benchmarks_power_closure %>%
  # filter(char_expression != "graphicalMCP closure Simes (R)") %>%
  plot_benchmarks("Power: 8,192 simulations", scale = TRUE)

df_benchmarks_power_closure %>%
  table_benchmarks("Power: 8,192 simulations", "s")
```
