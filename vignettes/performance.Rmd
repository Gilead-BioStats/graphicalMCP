...
plot_benchmarks <- function(df_benchmarks,
  ...
  aes(
    size,
    if (scale) median * 1e5 / sims else median,
  ...
  scale_y_log10(
    breaks = y_breaks,
    labels = function(breaks) format(breaks, scientific = FALSE, big.mark = ",")
  ) +
  labs(
...
df_benchmarks_gw %>%
  ...
  fmt_number(pattern = "{x}s") %>%
  tab_spanner("Number of hypotheses", 2:10) %>%
...
```{r df-benchmarks-power, fig.width=7, fig.height=5, warning=FALSE}
set.seed(7523)
...
if (file.exists(here("vignettes/data/df_benchmarks_power.rds"))) {
  df_benchmarks_power <-
    readRDS(here("vignettes/data/df_benchmarks_power.rds"))[keep_cols] %>% 
  ...
  list_benchmarks_power <- lapply(
    ...
    function(size) {
      ...
      speed_comp <- mark(
        `gMCP shortcut (C)` =
          calcPower(hyp, .025, trn, marg_pow, s_corr, n.sim = sims),
        `graphicalMCP shortcut (C++)` =
          calculate_power(rando, sim_n = sims, marginal_power = marg_pow,
                          sim_corr = s_corr),
        `gMCP closure parametric (R)` =
          if (size < 6) calcPower(hyp, .025, trn, marg_pow, s_corr, t_corr, sims),
        `graphicalMCP closure parametric (R)` =
          calculate_power(rando, sim_n = sims, marginal_power = marg_pow,
  ...
  df_benchmarks_power <- do.call(rbind, list_benchmarks_power)[keep_cols] %>% 
    filter(!(char_expression == "gMCP closure parametric (R)" & size > 5))
  
  saveRDS(
    df_benchmarks_power,
    here("vignettes/data/df_benchmarks_power.rds")
  )
}

df_benchmarks_power %>%
  filter(char_expression != "graphicalMCP closure Simes (R)") %>%
  plot_benchmarks("Power: 100,000 simulations", scale = TRUE)
```
