...
if (file.exists(here::here("vignettes/cache/power_benchmarks.csv"))) {
...
```{r plot-power-benchmarks-holm, eval=FALSE, fig.dim=c(8, 5)}
benchmarks_plot_standard <-
  ggplot(benchmarks, aes(num_hyps, median, color = char_expression)) +
  geom_point() +
  geom_line() +
...
benchmarks_plot_standard +
  ...
  labs(subtitle = "Log-10 median runtime for 65,536 simulations")
```
...
```{r plot-power-benchmarks-fs, eval=FALSE, fig.dim=c(8, 5)}
benchmarks_plot_standard <-
  ggplot(benchmarks, aes(num_hyps, median, color = char_expression)) +
  geom_point() +
  geom_line() +
