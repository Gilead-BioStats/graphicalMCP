...
compare the following approaches: `gMCP::generateWeights()`, `lrstat::fwgtmat()`, 
...
time (in median log-10 milliseconds) is plotted below. We can see that `gMCP::generateWeights()` 
is the slowest and `lrstat::fwgtmat()` is the fastest. Approach 2 
(graphicalMCP parent-child) is faster than Approach 1 (graphicalMCP simple). Note 
that `lrstat::fwgtmat()` implements the calculation using C++, which is known to be 
...
adding an Rcpp dependency is not considered worthwhile in `graphicalMCP`. For these considerations, 
