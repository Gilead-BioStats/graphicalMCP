...
- **C++ functions** - To be as fast as possible. Transparency is less important, except to the extent it impacts maintainability. These are all in support of power simulations primarily
  ...
  - bonferroni_sequential()
  - bonferroni_sequential_power_cpp()? May get folded into a general power function eventually
  - test_graph()
...
para_bench <- bench::mark(
  `para/Simes` = test_graph_fast(graph2, p, .05, list(1:2, 3:m), c("parametric", "simes"), intersections = gw, graph_size = m, gw_size = 2 ^ m - 1, num_groups = 2)
)

message(as.character(Sys.time()))
para_bench$expression <- as.character(para_bench$expression)
gt::gt(para_bench[1:9]) |> 
  tab_header("Blended parametric & Simes testing", paste(m, "hypotheses"))
```
...
power_bench <- bench::mark(
  ...
  # `Para/Simes - R, pre-calc gw only` = calculate_power(graph2, sim_n = sims, test_groups = list(1:2, 3:m), test_types = c("parametric", "simes"), test_corr = diag(m)), # blended power currently takes too long
  min_iterations = 3,
  check = FALSE
)
