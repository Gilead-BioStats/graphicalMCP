...
```{r bonferroni-mix-1}
test_graph(par_gate, p_values = c(.024, .01, .026, .027), alpha = .05)
```
...
There are two different testing functions - one which tests each hypothesis with the `p <= (c *) w * α` method, and another which calculates adjusted p-values. Currently the adjusted p-values method is more efficient. Using the `verbose = TRUE` option demonstrates the difference in information between the methods
...
# Does timing & checks results are equal
# Bonferroni
bench::mark(
  test_graph(graph2, p)$hypotheses_rejected,
  test_graph2(graph2, p)$hypotheses_rejected,
  gMCP(graph, p)@rejected
)

# Simes
bench::mark(
  test_graph(graph2, p, tests = list(simes = list(1:10)))$hypotheses_rejected,
  test_graph2(graph2, p, tests = "simes", groups = list(1:10))$hypotheses_rejected,
  gMCP(graph, p, test = "Simes")@rejected
)

# Parametric
bench::mark(
  test_graph(graph2, p, tests = list(parametric = list(1:10)), corr = corr)$hypotheses_rejected,
  test_graph2(graph2, p, tests = "parametric", groups = list(1:10), corr = corr)$hypotheses_rejected,
