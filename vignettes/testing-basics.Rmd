...
bonf_bench <- bench::mark(
  bonferroni_sequential(graph2, p)$outputs$p_adj,
  test_graph(graph2, p)$outputs$p_adj,
  test_graph(graph2, p, verbose = TRUE)$outputs$p_adj,
  test_graph(graph2, p, critical = TRUE)$outputs$p_adj,
  gMCP(graph, p)@adjPValues
)

bonf_bench$expression <- as.character(bonf_bench$expression)
rmarkdown::paged_table(bonf_bench)
```
...
simes_bench <- bench::mark(
  test_graph(graph2, p, test_types = "s")$outputs$p_adj,
  test_graph(graph2, p, test_types = "s", verbose = TRUE)$outputs$p_adj,
  test_graph(graph2, p, test_types = "s", critical = TRUE)$outputs$p_adj,
  gMCP(graph, p, test = "Simes")@adjPValues,
  check = FALSE # gMCP result is not named
)

simes_bench$expression <- as.character(simes_bench$expression)
rmarkdown::paged_table(simes_bench)
```
...
para_bench <- bench::mark(
  test_graph(graph2, p, test_types = "p", corr = sim_corr)$outputs$p_adj,
  test_graph(graph2, p, test_types = "p", corr = sim_corr, verbose = TRUE)$outputs$p_adj,
  test_graph(graph2, p, test_types = "p", corr = sim_corr, critical = TRUE)$outputs$p_adj,
  gMCP(graph, p, test = "parametric", correlation = sim_corr)@adjPValues
)

para_bench$expression <- as.character(para_bench$expression)
rmarkdown::paged_table(para_bench)
```
