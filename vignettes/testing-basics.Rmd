...
gwc <- add_critical_list(gw, diag(m), .05, list(1:m))
...
bonf_bench <- bench::mark(
  `graphicalMCP, C-like wrapped` =
    bs_cpp(graph2, p),
  `gMCP - graphTest (C)` =
    !!graphTest(p, graph = graph),
  `gMCP - C` =
    .C("graphproc", h = h, a = a, G = G, p, m, G, 1, 0, 0),
...
para_bench <- bench::mark(
  ...
  `Parametric 2` = test_graph_fast(graph2, p, .05, list(1:m), "parametric", intersections = gw, graph_size = m, gw_size = 2 ^ m - 1, num_groups = 1),
  `Parametric 3` = test_graph_fast_parametric(graph2, p, .05, list(1:m), "parametric", intersections = gw, inter_list = gwc, graph_size = m, gw_size = 2 ^ m - 1, num_groups = 1),
...
power_bench <- bench::mark(
  `gMCP` = calcPower(graph = graph, alpha = .05, n.sim = sims, corr.sim = diag(m)),
  # `graphicalMCP 4` = run_power4(graph2, n_sim = 1000),
  `graphicalMCP 5` = run_power5(graph2, n_sim = sims),
  min_iterations = 5,
  check = FALSE
)
...
power_bench <- bench::mark(
  `graphicalMCP 2` = run_power2(graph2, n_sim = sims / 1000, test_types = "simes"),
  `graphicalMCP 4` = run_power4(graph2, n_sim = sims / 1000, test_types = "simes"),
  `graphicalMCP 5` = run_power5(graph2, n_sim = sims / 1000, test_types = "simes"),
  min_iterations = 5,
  check = FALSE
)
...
# Lower number of sims just to compare methods
power_bench <- bench::mark(
  `gMCP (n.sim = 1)` = calcPower(graph = graph, alpha = .05, n.sim = 1, corr.sim = diag(m), corr.test = diag(m)), # n.sim = 10 took > 1 minute to run once
  `graphicalMCP 1` = run_power(graph2, n_sim = sims / 10000, test_types = "parametric", test_corr = diag(m)),
  `graphicalMCP 2` = run_power2(graph2, n_sim = sims / 10000, test_types = "parametric", test_corr = diag(m)),
  `graphicalMCP 5` = run_power5(graph2, n_sim = sims / 10000, test_types = "parametric", test_corr = diag(m)),
  min_iterations = 5,
  check = FALSE
)
...
```{r power-blend}
# fastest of each test type, all run with n_sim = 100K - takes a loooong time
power_bench <- bench::mark(
  `Bonferroni sequential - C++` = run_power5(graph2, n_sim = sims),
  `Simes - mixed language` = run_power4(graph2, n_sim = sims, test_types = "simes"),
  `Simes - mixed language (R ordered)` = run_power5(graph2, n_sim = sims, test_types = "simes"),
  # `Parametric - R` = run_power2(graph2, n_sim = sims, test_types = "parametric", test_corr = diag(m)),
  # `Multi-group, p/s` = run_power2(graph2, n_sim = sims, groups = list(1:2, 3:5), test_types = c("parametric", "simes"), test_corr = diag(m)),
  min_iterations = 3,
  check = FALSE
)
