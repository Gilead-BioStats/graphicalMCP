...
gwc <- graphicalMCP:::add_critical(gw, diag(m), .05, list(1:m))
gw_bonf <- graphicalMCP:::bonf_small(gw)
gw_para <- graphicalMCP:::add_critical2(gw, diag(m), .05, list(1:m))
...
gw_simes <- graphicalMCP:::simes_order4(gw_bonf, p, list(1:m))
...
bonf_bench <- bench::mark(
  ...
  `graphicalMCP, test_graph_fast (Simes)` = 
    test_graph_fast_simes_ordered_r(graph2, p, intersections = gw, test_types = "simes"),
...
```{r power-simes}
power_bench <- bench::mark(
  # `More general, less optimized` =
  #   calculate_power_q(graph2, sim_n = sims, test_types = "simes"),
...
power_bench <- bench::mark(
  ...
  # `calculate_power_q` =
  #   calculate_power_q(graph2, sim_n = sims, test_types = "parametric", test_corr = diag(m)),
  # `calculate_power` =
