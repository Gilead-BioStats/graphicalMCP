...
simes_bench <- bench::mark(
  ...
  `Simes 2` = test_graph_fast(graph2, p, .05, list(1:5), "simes", intersections = gw, graph_size = 5, gw_size = 31, num_groups = 1),
  `Simes 3` = test_graph_fast_simes(graph2, p, .05, list(1:5), "simes", intersections = gw, graph_size = 5, gw_size = 31, num_groups = 1),
  `Simes - gMCP` =
...
para_bench <- bench::mark(
  ...
  `Parametric 2` = test_graph_fast(graph2, p, .05, list(1:5), "parametric", intersections = gw, graph_size = 5, gw_size = 31, num_groups = 1),
...
power_bench <- bench::mark(
  `graphicalMCP 1` = run_power2(graph2, n_sim = sims / 10, test_types = "simes"),
  `graphicalMCP 2` = run_power2(graph2, n_sim = sims / 10, test_types = "simes"),
  `graphicalMCP 4` = run_power4(graph2, n_sim = sims / 10, test_types = "simes"),
  min_iterations = 5,
  check = FALSE
)
...
# Lower number of sims just to compare methods
power_bench <- bench::mark(
  `gMCP (n.sim = 1000)` = calcPower(graph = graph, alpha = .05, n.sim = 1, corr.sim = diag(m), corr.test = diag(m)), # n.sim = 10 took > 1 minute to run once
  `graphicalMCP 1` = run_power(graph2, n_sim = sims / 100, test_types = "parametric", test_corr = diag(m)),
  `graphicalMCP 2` = run_power2(graph2, n_sim = sims / 100, test_types = "parametric", test_corr = diag(m)),
  min_iterations = 5,
  check = FALSE
)
