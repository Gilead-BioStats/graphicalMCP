...
gw_para <- calculate_critical_parametric(gw_bonf, diag(m), .05, list(1:m))

p <- runif(m, .0001, .03)
sim_corr <- diag(m)

gw_simes <- calculate_critical_simes(gw_bonf, p, list(1:m))
...
bonf_bench <- bench::mark(
  `C++ sequential` =
    as.logical(bonferroni_sequential_cpp(graph2, p)),
  `closure (vec/matrixStats)` =
    as.logical(test_graph_fast(p, alpha = .05, intersections = gw_bonf)),
  `gMCP, raw C` =
...
simes_bench <- bench::mark(
  `graphicalMCP` =
    test_graph_fast(
      p,
      .05,
      calculate_critical_simes(gw_bonf, p, list(1:m))
    ),
  `gMCP::gMCP` =
...
para_bench <- bench::mark(
  `vec/matrixStats (no critical)` =
    test_graph_fast(p, alpha = .05, intersections = gw_para),
  `gMCP::graphTest` =
