...
df_gw <- graphicalMCP:::generate_weights2(graph2)
...
bonf_bench <- bench::mark(
  `graphicalMCP, C++ with R wrapper` =
    bs_cpp(graph2, p),
  `graphicalMCP, closure` = 
    test_graph_df(df_gw, p),
  ...
  `graphicalMCP, test_graph_fast (parametric)` = 
    test_graph_fast(graph2, p, intersections = gw, test_types = "parametric", corr = diag(3)),
