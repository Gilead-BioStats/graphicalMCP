...
bonf_bench <- bench::mark(
  ...
  `graphicalMCP, test_graph_fast_v (bonf)` = 
    test_graph_fast_v(graph2, p, intersections = gw_bonf),
  `graphicalMCP, test_graph_fast_v (para)` = 
    test_graph_fast_v(graph2, p, intersections = gw_para),
  `graphicalMCP, test_graph_fast_v (simes)` = 
    test_graph_fast_v(graph2, p, intersections = gw_simes),
...
power_bench <- bench::mark(
  ...
  `calculate_power_v` =
    calculate_power_v(graph2, sim_n = sims),
  `calcPower` =
...
power_bench <- bench::mark(
  ...
  `calculate_power_v` =
    calculate_power_v(graph2, sim_n = sims, test_types = "parametric", test_corr = diag(m)),
  min_iterations = 1,
  filter_gc = FALSE,
  check = FALSE
)
...
power_bench <- bench::mark(
  ...
  `Para/Simes vectorized` = 
    calculate_power_v(graph2, sim_n = sims, test_groups = list(1:2, 3:m), test_types = c("p", "s"), test_corr = diag(m)),
  min_iterations = 1,
  filter_gc = FALSE,
  check = FALSE
)
...
# power_bench <- bench::mark(
#   `calculate_power_q` = 
#     calculate_power_q(graph2, sim_n = sims, test_groups = list(1:2, 3:m), test_types = c("parametric", "simes"), test_corr = diag(m)),
#   `calculate_power` = 
#     calculate_power(graph2, sim_n = sims, test_groups = list(1:2, 3:m), test_types = c("parametric", "simes"), test_corr = diag(m)),
#   min_iterations = 1,
#   filter_gc = FALSE,
