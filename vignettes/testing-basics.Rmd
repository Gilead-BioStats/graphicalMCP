...
bonf_bench <- bench::mark(
  ...
  # `Bonferroni sequential2 (C++)` =
  #   bonferroni_sequential2(graph2, p)$outputs$rejected,
  # `Bonferroni sequential3 (C++)` =
  #   bonferroni_sequential3(graph2, p)$outputs$rejected, # currently wrong
  # `Bonferroni sequential, critical` =
  #   bonferroni_sequential(graph2, p, critical = TRUE)$outputs$rejected,
  # `Bonferroni sequential, C-like` =
  #   graphproc(
  #     h = h,
  #     a = a,
  #     G = G,
  ...
  #     upscale = 0
  #   ),
  `Bonferroni sequential, C-like wrapped` =
    bonferroni_sequential4(graph2, p),
  # `Bonferroni closure` =
...
bench::mark(
  `gMCP` = calcPower(graph = graph, alpha = .05, n.sim = 100000, corr.sim = diag(m)),
  # `graphicalMCP 4` = run_power4(graph2, n_sim = 1000),
  `graphicalMCP 5` = run_power5(graph2, n_sim = 100000),
  min_iterations = 5,
  check = FALSE
)
...
bench::mark(
  # `gMCP` = calcPower(graph = graph, alpha = .05, n.sim = 100000, corr.sim = diag(m)),
  `graphicalMCP` = run_power(graph2, n_sim = 100, test_types = "simes"),
  `graphicalMCP 2` = run_power2(graph2, n_sim = 100, test_types = "simes"),
  min_iterations = 5,
  check = FALSE
)

bench::mark(
  `gMCP` = calcPower(graph = graph, alpha = .05, n.sim = 100, corr.sim = diag(m), test.sim = diag(m)),
  `graphicalMCP` = run_power(graph2, n_sim = 100, test_types = "parametric", test_corr = diag(m)),
  min_iterations = 5,
  check = FALSE
)
```
