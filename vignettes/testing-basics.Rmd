...
gwc <- add_critical(gw, diag(m), .05, list(1:m))
gw_bonf <- ifelse(
  gw[, seq_len(ncol(gw) / 2)],
  gw[, seq_len(ncol(gw) / 2) + (ncol(gw) / 2)],
  NA
)
gw_para <- calculate_critical_parametric2(gw_bonf, diag(m), .05, list(1:m))

p <- runif(m, .0001, .03)
sim_corr <- diag(m)

gw_simes <- calculate_critical_simes_vms(gw_bonf, p, list(1:m))
...
bonf_bench <- bench::mark(
  ...
  `graphicalMCP, test_graph_fast` =
    test_graph_fast(graph2, p, intersections = gw),
  `graphicalMCP, test_graph_fast (Simes)` =
    tg_fast_simes_ord_r(graph2, p, intersections = gw, test_types = "simes"),
  `graphicalMCP, test_graph_fast (parametric)` =
    test_graph_fast(graph2, p, intersections = gw, test_types = "parametric", corr = diag(m)),
  `graphicalMCP, test_graph_fast_v (bonf)` =
    graphicalMCP:::test_graph_fast_v(p, alpha = .05, intersections = gw_bonf),
  `graphicalMCP, test_graph_fast_v (para)` =
    graphicalMCP:::test_graph_fast_v(p, alpha = .05, intersections = gw_para),
  `graphicalMCP, test_graph_fast_v (simes)` =
    graphicalMCP:::test_graph_fast_v(p, alpha = .05, intersections = gw_simes),
  `graphicalMCP, test_graph_fast_vms (bonf)` =
    graphicalMCP:::test_graph_fast_vms(p, alpha = .05, intersections = gw_bonf),
  `graphicalMCP, test_graph_fast_vms (para)` =
    graphicalMCP:::test_graph_fast_vms(p, alpha = .05, intersections = gw_para),
  `graphicalMCP, test_graph_fast_vms (simes)` =
    graphicalMCP:::test_graph_fast_vms(p, alpha = .05, intersections = gw_simes),
...
simes_bench <- bench::mark(
  ...
  `C++ test, gw pre, sort once` =
    tg_fast_simes_ord_cpp(graph2, p, .05, list(1:m), "simes", intersections = gw, graph_size = m, gw_size = 2^m - 1, num_groups = 1),
  `R test, gw pre, sort once` =
    tg_fast_simes_ord_r(graph2, p, .05, list(1:m), "simes", intersections = gw, graph_size = m, gw_size = 2^m - 1, num_groups = 1),
  `test_graph_fast_v (simes-1)` =
    graphicalMCP:::test_graph_fast_v(p, alpha = .05, intersections = graphicalMCP:::calculate_critical_simes(gw_bonf, p, list(1:m))),
  `test_graph_fast_v (simes-2)` =
    graphicalMCP:::test_graph_fast_v(p, alpha = .05, intersections = graphicalMCP:::calculate_critical_simes(gw_bonf, p, groups)),
  `test_graph_fast_vms (simes-1)` =
    graphicalMCP:::test_graph_fast_vms(p, alpha = .05, intersections = graphicalMCP:::calculate_critical_simes_vms(gw_bonf, p, list(1:m))),
  `test_graph_fast_vms (simes-2)` =
    graphicalMCP:::test_graph_fast_vms(p, alpha = .05, intersections = graphicalMCP:::calculate_critical_simes_vms(gw_bonf, p, groups)),
  `gMCP::gMCP` =
...
para_bench <- bench::mark(
  ...
  `gw pre, critical pre` =
    test_graph_fast_para(graph2, p, .05, list(1:m), "parametric", intersections = gw, inter_list = gwc, graph_size = m, gw_size = 2^m - 1, num_groups = 1),
  `test_graph_fast_v (para)` =
    graphicalMCP:::test_graph_fast_v(p, alpha = .05, intersections = gw_para),
  `test_graph_fast_vms (para)` =
    graphicalMCP:::test_graph_fast_vms(p, alpha = .05, intersections = gw_para),
  `testGraph` =
...
power_bench <- bench::mark(
  ...
  `calculate_power_vms` =
    graphicalMCP:::calculate_power_vms(graph2, sim_n = sims),
  `calcPower` =
...
power_bench <- bench::mark(
  ...
  `calculate_power_vms` =
    graphicalMCP:::calculate_power_vms(graph2, sim_n = sims, test_types = "s"),
  min_iterations = 1,
  filter_gc = FALSE,
  check = FALSE
)
...
power_bench <- bench::mark(
  ...
  # `gw & adj-p each time` =
  #   calc_power_slow(graph2, sim_n = sims, test_types = "parametric", test_corr = diag(m)),
  # `calculate_power` =
  #   calculate_power(graph2, sim_n = sims, test_types = "parametric", test_corr = diag(m)),
  `calculate_power_vms` =
    graphicalMCP:::calculate_power_vms(graph2, sim_n = sims, test_types = "p", test_corr = diag(m)),
  min_iterations = 1,
  filter_gc = FALSE,
  check = FALSE
)
...
power_bench <- bench::mark(
  # `Bonferroni sequential - C++` =
  #   calculate_power(graph2, sim_n = sims),
  # `Bonferroni closure - R` =
  #   graphicalMCP:::calculate_power_vms(graph2, sim_n = sims),
  # `Simes vectorized` =
  #   graphicalMCP:::calculate_power_vms(graph2, sim_n = sims, test_types = "simes"),
  # `Parametric vectorized` =
  #   graphicalMCP:::calculate_power_vms(graph2, sim_n = sims, test_types = "parametric", test_corr = diag(m)),
  # `Para/Simes - R, pre-calc gw only` =
  #   calculate_power(graph2, sim_n = sims, test_groups = list(1:2, 3:m), test_types = c("parametric", "simes"), test_corr = diag(m)),
  `Para/Simes vectorized` =
    graphicalMCP:::calculate_power_vms(graph2, sim_n = sims, test_groups = list(1:floor(m / 2), (floor(m / 2) + 1):(m - 1), m), test_types = c("p", "s", "b"), test_corr = diag(m)),
  min_iterations = 1,
  filter_gc = FALSE,
  check = FALSE
)
...
power_bench <- bench::mark(
  `para_groups` =
    graphicalMCP:::calculate_power_vms(graph2, sim_n = sims, test_groups = list(1:floor(m / 2), (floor(m / 2) + 1):m), test_types = c("p", "p"), test_corr = diag(m))
)
...
power_bench <- bench::mark(
  `simes_groups` =
    graphicalMCP:::calculate_power_vms(graph2, sim_n = sims, test_groups = list(1:2, 3:m), test_types = c("s", "s"), test_corr = diag(m))
)
