...
- **C++ functions** - To be as fast as possible. Transparency is less important, except to the extent it impacts maintainability. These are all in support of power simulations primarily
  - bonferroni_sequential_cpp()
  - bonferroni_sequential_power_cpp()
  - test_graph() is fastest in R using vectorization or matrixStats
  - calculate_power() is currently fastest using a blend of languages - C++ for sequential Bonferroni testing, and R for the more complex closure testing
...
bonf_bench <- bench::mark(
  `C++ sequential` =
    as.logical(bs_cpp(graph2, p)),
  `closure` =
    as.logical(test_graph_fast(graph2, p, intersections = gw)),
  `closure (vectorized)` =
    as.logical(test_graph_fast_v(p, alpha = .05, intersections = gw_bonf)),
  `closure (vec/matrixStats)` =
    as.logical(test_graph_fast_vms(p, alpha = .05, intersections = gw_bonf)),
  `gMCP, raw C` =
    .C("graphproc", h = h, a = a, G = G, p, m, G, 1, 0, 0)$h, # different result?
  check = FALSE
)
...
simes_bench <- bench::mark(
  `adj-p` =
    test_graph_fast(graph2, p, .05, list(1:m), "simes", intersections = gw),
  `adj-p, C++` =
    test_graph_fast_simes(graph2, p, .05, list(1:m), intersections = gw),
  `vectorized, 1 group` =
    test_graph_fast_v(p, .05, calculate_critical_simes(gw_bonf, p, list(1:m))),
  `vectorized, 2 groups` =
    test_graph_fast_v(p, .05, calculate_critical_simes(gw_bonf, p, groups)),
  `vec/matrixStats, 1 group` =
    test_graph_fast_vms(p, .05, calculate_critical_simes_vms(gw_bonf, p, list(1:m))),
  `vec/matrixStats, 2 groups` =
    test_graph_fast_vms(p, .05, calculate_critical_simes_vms(gw_bonf, p, groups)),
  `gMCP::gMCP` =
...
para_bench <- bench::mark(
  `adj-p` =
    test_graph_fast(graph2, p, .05, list(1:m), "parametric", intersections = gw),
  `adj-p (no critical)` =
    test_graph_fast_para(graph2, p, .05, list(1:m), intersections = gw, inter_list = gwc),
  `vectorized (no critical)` =
    test_graph_fast_v(p, alpha = .05, intersections = gw_para),
  `vec/matrixStats (no critical)` =
    test_graph_fast_vms(p, alpha = .05, intersections = gw_para),
  `gMCP::graphTest` =
...
```{r power-bonf, cache=TRUE}
sims <- params$sims

power_bench <- bench::mark(
  `graphicalMCP` =
    calculate_power_vms(graph2, sim_n = sims),
  `gMCP` =
...
```{r power-simes, cache=TRUE}
power_bench <- bench::mark(
  `graphicalMCP (vectorized)` =
    calculate_power_v(graph2, sim_n = sims, test_types = "s"),
  `graphicalMCP (vec/matrixStats)` =
    calculate_power_vms(graph2, sim_n = sims, test_types = "s"),
  min_iterations = 5,
  filter_gc = FALSE,
  check = FALSE
)
...
```{r power-para, cache=TRUE}
power_bench <- bench::mark(
  # `gMCP (n.sim = 1)` =
  #   calcPower(graph = graph, alpha = .05, n.sim = 1, corr.sim = diag(m), corr.test = diag(m)),
  `graphicalMCP (vectorized)` =
    calculate_power_v(graph2, sim_n = sims, test_types = "p", test_corr = diag(m)),
  `graphicalMCP (vec/matrixStats)` =
    calculate_power_vms(graph2, sim_n = sims, test_types = "p", test_corr = diag(m)),
  min_iterations = 5,
  filter_gc = FALSE,
  check = FALSE
)
