...
G <- as.double(graph2$transitions)
nH <- m
```
...
bonf_bench <- bench::mark(
  `C++ sequential` =
    as.logical(graphicalMCP:::bonferroni_sequential_cpp(graph2, p)),
  `closure (vec/matrixStats)` =
    as.logical(graphicalMCP:::test_graph_fast(p, .05, gw_bonf, inter_h)),
  `gMCP, raw C` =
    .C("graphproc", h = h, a = a, G = G, p, m, G, 1, 0, 0)$h, # different val?
  `gMCP::gMCP` =
...
power_bench <- bench::mark(
  ...
  `graphicalMCP (vec/matrixStats)` =
    calculate_power(graph2, sim_n = sims, test_types = "p", test_corr = diag(m)),
  min_iterations = 3,
  filter_gc = FALSE,
  check = FALSE
)
