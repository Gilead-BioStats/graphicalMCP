...
Power values will differ slightly due to different simulations, but on the whole they are very similar. And that's all we need to do! Furthermore, once we've done the hard work of setting up the scenarios, it's simple to test them using other strategies. Let's try the Simes method. Note that Simes testing takes \~10x longer than Bonferroni testing for a graph of this size.
