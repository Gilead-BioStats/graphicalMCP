...
setMethod(
    "edgeAttr", signature(
        self = "graphMCP", from = "character", to = "character",
        attr = "character"
    ),
...
setMethod(
    "nodeAttr", signature(self = "graphMCP", n = "character", attr = "character"),
...
getFractionString <- function(x, eps = 1e-07) {
    ...
    xStr <- ifelse(abs(sapply(xStr, function(x) {
        eval(parse(text = x))
    }) - x) > eps, as.character(x), xStr)
    return(xStr)
}
...
setMethod("simConfint", c("graphMCP"), function(object, pvalues, confint, alternative = c("less", "greater"), estimates, df, alpha = 0.05, mu = 0) {
    ...
    if (class(confint) == "function") {
        f <- function(node, alpha, rejected) {
            if (rejected && alternative == "less") {
                return(c(-Inf, mu))
            }
            if (rejected && alternative == "greater") {
                return(c(mu, Inf))
            }
