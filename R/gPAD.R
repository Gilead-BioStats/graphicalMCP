...
adaptWeights <- function(J, select, G2, dw = "reject") {
    ## adapt the weights this is basically a wrapper to CalculateWeights that handles dropped hypotheses
    w <- getWeights(G2)
    g <- graph2matrix(G2)
    ## in case we only include selected hypotheses
    if (all((J - select) > 0)) {
        return(CalculateWeights(J, g, w))
    }
    ## canonical rule number 1
    if (dw == "reject") {
        return(CalculateWeights(J * select, g, w))
    }
    ## canonical rule number 2 with fallback to 1 in case all weights are zero
    if (dw == "accept") {
        if (all(w <- CalculateWeights(J, g, w) * select) == 0) {
            return(CalculateWeights(J * select, g, w))
        } else {
            return(w)
        }
    ...
    if (dw == "strict") {
        return(CalculateWeights(J, g, w) * select)
    }
    ##
