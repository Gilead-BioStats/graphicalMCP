...
startGUI <- function() {
    initJava()
    if (!"jri.jar" %in% tolower(sapply(.jclassPath(), function(x) {
        substring(x, first = nchar(x) - 6)
    }))) {
        warning(paste(c(
            "JRI.jar seems to be missing from the classpath. ",
            ...
            "and reinstall rJava to use JRI functionality."
        ), collapse = "\n"))
    }
