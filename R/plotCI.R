...
#'
#' ci <- simConfint(BonferroniHolm(3),
#'     pvalues = pval,
...
plotSimCI <- function(ci) {
    ...
    if (is.null(lower) & is.null(upper)) {
        warning("No confidence limits specified: arguments lower and upper are both empty!")
    }

    # define the plot range

    allpoints <- c(lower, estimate, upper)
    if (all(!is.finite(allpoints))) {
        stop("Arguments estimate, lower and upper contain only infinity or missing values!")
    }
    ...
    for (i in 1:length(num)) {
        arrows(
            y0 = rnum[i], y1 = rnum[i], x0 = lower[i], x1 = upper[i],
