...
BonferroniHolm <- function(n, weights = rep(1 / n, n)) {
    if (missing(n)) {
        stop("Please provide the number of hypotheses as parameter n.")
    }
    ...
    attr(BonferroniHolm, "description") <- paste("Graph representing the (unweighted) Bonferroni-Holm-Procedure",
        ...
        "",
        "Literature: Holm, S. (1979). A simple sequentally rejective multiple test procedure. Scandinavian Journal of Statistics 6, 65-70.",
        sep = "\n"
    )
...
BretzEtAl2011 <- function() {
    ...
    attr(graph, "description") <- paste("Graph representing the procedure from Bretz et al. (2011) - Figure 2",
        ...
        "",
        "Literature: Bretz, F., Maurer, W. and Hommel, G. (2011), Test and power considerations for multiple endpoint analyses using sequentially rejective graphical procedures. Statistics in Medicine, 30: 1489-1501.",
        sep = "\n"
    )
    return(graph)
}
...
BauerEtAl2001 <- function() {
    ...
    attr(graph, "description") <- paste("Graph representing the procedure from Bretz et al. (2011) - Figure 2",
        ...
        "",
        "Bretz F., Maurer W., Brannath W., Posch M.: A graphical approach to sequentially rejective multiple test procedures. Statistics in Medicine 2009; 28:586-604. Figure 8.",
        sep = "\n"
    )
    return(graph)
}
...
BretzEtAl2009c <- function() {
    ...
    attr(graph, "description") <- paste("Graph from Bretz et al. (2009) - Figure 15",
        "",
        "Literature: Bretz F., Maurer W., Brannath W., Posch M.: A graphical approach to sequentially rejective multiple test procedures. Statistics in Medicine 2009; 28:586-604. Figure 15.",
        sep = "\n"
    )
    return(graph)
}
...
HommelEtAl2007 <- function() {
    ...
    attr(graph, "description") <- paste("Graph representing the procedure from Hommel et al. (2007)",
        ...
        "",
        "Literature: Hommel, G., Bretz, F. und Maurer, W. (2007). Powerful short-cuts for multiple testing procedures with special reference to gatekeeping strategies. Statistics in Medicine, 26(22), 4063-4073.",
        sep = "\n"
    )
...
parallelGatekeeping <- function() {
    ...
    attr(graph, "description") <- paste("Graph representing a parallel gatekeeping procedure from Dmitrienko et al. (2003) Table I",
        "",
        "Literature: Dmitrienko, A., Offen, W., Westfall, P.H. (2003). Gatekeeping strategies for clinical trials that do not require all primary effects to be significant. Statistics in Medicine. 22, 2387-2400.",
        sep = "\n"
    )
    return(graph)
}
...
improvedParallelGatekeeping <- function() {
    ...
    attr(graph, "description") <- paste("Graph representing an improved parallel gatekeeping procedure",
        "",
        "Literature: Bretz, F., Maurer, W., Brannath, W., Posch, M.: A graphical approach to sequentially rejective multiple test procedures. Statistics in Medicine 2009 vol. 28 issue 4 page 586-604. URL: http://www.meduniwien.ac.at/fwf_adaptive/papers/bretz_2009_22.pdf .",
        sep = "\n"
    )
    return(graph)
}
...
fixedSequence <- function(n) {
    ...
    attr(graph, "description") <- paste("Graph representing the fixed sequence test",
        ...
        "",
        "Westfall P.H., Krishen A.: Optimally weighted, fixed sequence, and gatekeeping multiple testing procedures. Journal of Statistical Planning and Inference 2001; 99:25-40.",
        sep = "\n"
    )
    return(graph)
}
...
simpleSuccessiveI <- function() {
    ...
    attr(graph, "description") <- paste("Simple successive graph from Maurer et al. (2011)",
        ...
        "",
        "F. Bretz, M. Posch, E. Glimm, F. Klinglmueller, W. Maurer, K. Rohmeyer (2011), Graphical approaches for multiple comparison procedures using weighted Bonferroni, Simes or parametric tests. To be published. Figure 12.",
        sep = "\n"
    )
    return(graph)
}
...
truncatedHolm <- function(gamma) {
    ...
    attr(graph, "description") <- paste("Example of the Truncated Holm Procedure",
        ...
        "",
        "F. Bretz, M. Posch, E. Glimm, F. Klinglmueller, W. Maurer, K. Rohmeyer (2011), Graphical approaches for multiple comparison procedures using weighted Bonferroni, Simes or parametric tests. To be published. Figure 5.",
        sep = "\n"
    )
    if (!missing(gamma)) {
...
HuqueAloshEtBhore2011 <- function() {
    ...
    attr(graph, "description") <- paste("Graph representing the procedure from Huque, Alosh and Bhore (2011)",
        "",
        "Literature: Huque, M.F. and Alosh, M. and Bhore, R. (2011), Addressing Multiplicity Issues of a Composite Endpoint and Its Components in Clinical Trials. Journal of Biopharmaceutical Statistics, 21: 610-634.",
        sep = "\n"
    )
    return(graph)
}
...
HungEtWang2010 <- function(nu, tau, omega) {
    ...
    attr(graph, "description") <- paste("Graph representing the procedure from Hung and Wang (2010)",
        ...
        "",
        "Literature: Hung H.M.J., Wang S.-J. (2010). Challenges to multiple testing in clinical trials. Biometrical Journal 52, 747-756.",
        sep = "\n"
    )
    variables <- list()
...
FerberTimeDose2011 <- function(times, doses, w = "\\nu") {
    ...
    attr(graph, "description") <- paste("Second graph from Ferber et al. 2011",
        "",
        "Literature: G. Ferber, L. Staner and P. Boeijinga (2011): Structured multiplicity and confirmatory statistical analyses in pharmacodynamic studies using the quantitative electroencephalogram, Journal of neuroscience methods, Volume 201, Issue 1, Pages 204-212.",
        sep = "\n"
    )
...
Ferber2011 <- function(w) {
    # Nodes:
    hnodes <- c(
        "\\delta", "\\theta", "\\beta", "\\alpha", "\\alpha_1",
        "\\alpha_2", "\\beta_1", "\\beta_2", "\\beta_3"
    )
    # Edges:
    m <- matrix(c(
        "0", "1/3", "1/6", "1/6", "0", "0", "0", "0", "0",
    ...
    graph@edgeAttr$labelX <- structure(c(
        NA, 242, 296, 62, NA, NA, NA, NA, NA, 196, NA, 333,
        ...
        NA, NA, 377, NA, NA, NA, 291, 439, NA
    ), .Dim = c(9L, 9L), .Dimnames = list(
        c(
            "\\delta", "\\theta", "\\beta", "\\alpha", "\\alpha_1",
            "\\alpha_2", "\\beta_1", "\\beta_2", "\\beta_3"
        ), c(
    ...
    graph@edgeAttr$labelY <- structure(c(
        NA, 39, 131, 120, NA, NA, NA, NA, NA, 10, NA, 121,
...
Entangled1Maurer2012 <- function() {
    ...
    attr(graph, "description") <- paste("Graph from Maurer and Bretz 2012",
        ...
        "",
        "Literature: Maurer, W., & Bretz, F. (2013). Memory and other properties of multiple test procedures generated by entangled graphs. Statistics in medicine, 32(10), p. 1739-1753.",
        sep = "\n"
    )
