...
checkOptimal <- function(graph, verbose = TRUE) {
    if (any(is.na(parse2numeric(graph, force = TRUE)@m))) {
        return("Graph contains variables (or unparsable terms). No analysis possible.")
    }
    ...
    if (!isTRUE(all.equal(rowSums(all.weights), rep(1, dim(all.weights)[1])))) {
        ...
        if (isTRUE(all.equal(sum(getWeights(graph)), 1))) {
            wrong <- unlist(lapply(rowSums(all.weights), function(x) {
