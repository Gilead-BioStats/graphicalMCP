...
```{r bonferroni-mix-1}
test_graph_closure(par_gate, p = pvals, alpha = .05)
```
...
```{r simes-all-1}
test_graph_closure(par_gate, p = pvals, alpha = .05, test_types = "s")
```
...
`test_graph_closure()` always calculates the full closure, even for Bonferroni testing. If optimal performance is needed, `test_graph_shortcut()` can be used, which uses the sequential testing shortcut. This causes a significant improvement in speed, and either method is faster than `gMCP::gMCP()`. However, using `gMCP::graphTest()` is the fastest option, as it's written primarily in C directly.
...
- **C++ functions** - To be as fast as possible. Transparency is less important, except to the extent it impacts maintainability. These are all in support of power simulations primarily
  - test_graph_shortcut_cpp()
  - power_shortcut_cpp()
  - test_graph_closure() is fastest in R using vectorization or matrixStats
...
gw_para <-
  graphicalMCP:::calculate_critical_parametric(gw_bonf, diag(m), .05, list(1:m))

p <- runif(m, .0001, .03)
sim_corr <- diag(m)

gw_simes <- graphicalMCP:::calculate_critical_simes(gw_bonf, p, list(1:m))
...
bonf_bench <- bench::mark(
  `C++ sequential` =
    as.logical(graphicalMCP:::test_graph_shortcut_cpp(graph2, p)),
  `closure (vec/matrixStats)` =
    as.logical(graphicalMCP:::test_graph_fast(p, .05, gw_bonf, inter_h)),
  `gMCP, raw C` =
...
simes_bench <- bench::mark(
  `graphicalMCP` =
    graphicalMCP:::test_graph_fast(
      p,
      .05,
      graphicalMCP:::calculate_critical_simes(gw_bonf, p, list(1:m)),
      inter_h
    ),
  `gMCP::gMCP` =
...
para_bench <- bench::mark(
  `vec/matrixStats (no critical)` =
    graphicalMCP:::test_graph_fast(p, alpha = .05, gw_para, inter_h),
  `gMCP::graphTest` =
...
power_bench <- bench::mark(
  `graphicalMCP (vec/matrixStats)` =
    calculate_power(graph2, sim_n = sims, test_types = "s"),
  min_iterations = 3,
  filter_gc = FALSE,
  check = FALSE
)
